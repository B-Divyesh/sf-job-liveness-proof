...
fn default_database_url() -> String {
    if Path::new("/data").is_dir() { "sqlite:///data/run-proof.db?mode=rwc".into() } else { "sqlite://run-proof.db?mode=rwc".into() }
}

#[derive(Debug, Clone, Copy)]
enum SettingSource { Supplied, Defaulted, Generated, Persisted }

impl SettingSource {
    fn as_str(self) -> &'static str {
        match self { Self::Supplied => "supplied", Self::Defaulted => "defaulted", Self::Generated => "generated", Self::Persisted => "persisted" }
    }
}

fn secret_file_for(database_url: &str) -> PathBuf {
    let path = database_url.strip_prefix("sqlite://").unwrap_or(database_url).split('?').next().unwrap_or("run-proof.db");
    let database_path = Path::new(path);
    database_path.parent().filter(|parent| !parent.as_os_str().is_empty()).unwrap_or_else(|| Path::new(".")).join("run-proof.secret")
}

fn load_or_create_secret(path: &Path) -> io::Result<(String, SettingSource)> {
    match fs::read_to_string(path) {
        Ok(secret) if secret.trim().len() >= 32 => Ok((secret.trim().to_owned(), SettingSource::Persisted)),
        Ok(_) => Err(io::Error::new(io::ErrorKind::InvalidData, "persisted RUN_PROOF_SECRET is shorter than 32 characters")),
        Err(error) if error.kind() == io::ErrorKind::NotFound => {
            if let Some(parent) = path.parent() { fs::create_dir_all(parent)?; }
            let mut bytes = [0_u8; 32];
            getrandom::fill(&mut bytes).map_err(|error| io::Error::other(format!("could not generate RUN_PROOF_SECRET: {error}")))?;
...
fn configured_secret(database_url: &str) -> Result<(String, SettingSource), String> {
    match env::var("RUN_PROOF_SECRET") {
        ...
        Err(env::VarError::NotPresent) => load_or_create_secret(&secret_file_for(database_url)).map_err(|error| format!("could not load or generate RUN_PROOF_SECRET: {error}")),
...
async fn main() {
    ...
    let database_source = if env::var_os("DATABASE_URL").is_some() { SettingSource::Supplied } else { SettingSource::Defaulted };
    let database=env::var("DATABASE_URL").unwrap_or_else(|_|default_database_url());
    let (secret, secret_source)=match configured_secret(&database) { Ok(configured) => configured, Err(error) => { warn!(%error, "Run Proof configuration error"); return; } };
    ...
    info!(database = database_source.as_str(), secret = secret_source.as_str(), key_id = if env::var_os("RUN_PROOF_KEY_ID").is_some() { "supplied" } else { "defaulted" }, "Run Proof runtime configuration");
    let pool=match connect(&database).await { Ok(pool) => pool, Err(error) => { warn!(%error, "Run Proof database connection failed"); return; } };
    if let Err(error)=apply_retention(&pool,retention).await { warn!(%error, "Run Proof retention cleanup failed"); return; }
